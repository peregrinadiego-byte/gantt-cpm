...
function renderActivityTable(rows){
  ...
  rows.forEach((r,i)=>{
    ...
    tr.innerHTML=`
      ...
      <td><button class="remove-btn" data-remove="${i}" title="Eliminar">×</button></td>`;
  ...
  tbody.querySelectorAll('[data-remove]').forEach(btn=>btn.addEventListener('click',()=>{
    activities.splice(+btn.dataset.remove,1); render();
  }));
}
...
function renderNetwork(rows){
  ...
  const levels={}; rows.forEach(r=>{ const lvl=r.es; (levels[lvl] ||= []).push(r); });
  // derive compact level index based on topological milestones rather than absolute ES
  const levelByKey={};
...
startInput.addEventListener('change',render); document.getElementById('calendarMode').addEventListener('change',render);
